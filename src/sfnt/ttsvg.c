...
#define FT_COMPONENT  ttsvg
                        ...
                        FT_UInt       glyph_index )
  {
                                 ...
                                 && ( doc_list[2] == 0x08 ) )
    {
       ...
       * buffer to accomodate the uncompressed version.  The last 4 bytes of
       * the compressed document are equal to original_size modulo 2^32.
       * Since the size of SVG documents is less than 2^32 bytes we can use
       * this accurately.  The four bytes are stored in little-endian format.
       */
